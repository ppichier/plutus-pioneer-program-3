...
mkValidator p dat () cxt = traceIfFalse "beneficiary's signature missing" signedByBeneficiary &&
                           traceIfFalse "deadline not reached" deadlineReached  -- FIX ME!
    where
        info :: TxInfo
...
typedValidator p = Scripts.mkTypedValidator @Vesting -- IMPLEMENT ME!
