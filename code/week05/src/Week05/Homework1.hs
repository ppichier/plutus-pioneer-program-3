...
mkPolicy pkh deadline () ctx = signedByOwnerOfPkh && deadlineNotReached -- FIX ME!
    where
        info :: TxInfo
        info = scriptContextTxInfo ctx

        signedByOwnerOfPkh :: Bool
        signedByOwnerOfPkh = txSignedBy info $ unPaymentPubKeyHash pkh
...
policy pkh deadline = mkMintingPolicyScript $ -- IMPLEMENT ME!
...
curSymbol pkh deadline = scriptCurrencySymbol $ policy pkh deadline -- IMPLEMENT ME!
...
mint mp = do
    ...
    if now > deadline
        then Contract.logError @String "deadline passed"
            ...
            let val     = Value.singleton (curSymbol pkh deadline) (mpTokenName mp) (mpAmount mp) -- compute the value i want to mint
                lookups = Constraints.mintingPolicy $ policy pkh deadline
                tx      = Constraints.mustMintValue val <> Constraints.mustValidateIn (to $ now + 60000) -- as long as this tx happens to the blockchain in at most a minute
